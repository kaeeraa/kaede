...
- `app.windows[0].title` is just a visible title for the window title bar, but it servers an important purpose in Kaede. If launcher build will be portable, then that `title` property must contain a `Portable` string. This is required, because in the logging file initialization (`lib.rs`) I determine whether launcher is portable or not by checking if window title contains a `Portable` string.
